...
# Gap handling for breaker daily energy sensors.
...
GAP_HANDLING_SKIP = "skip"           # Don't accumulate energy during the gap (under-reports on network loss, correct on power loss)
GAP_HANDLING_EXTRAPOLATE = "extrapolate"  # Use last known power across the gap (correct on network loss if loads stable, over-reports on power loss)
GAP_HANDLING_AVERAGE = "average"     # Average last known + recovery power across the gap (compromise)
